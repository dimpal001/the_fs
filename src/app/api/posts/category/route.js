...
export async function GET(request, { params }) {
  ...
  try {
    ...
    const [posts] = await db.query(
      `SELECT BlogPosts.id, BlogPosts.title, BlogPosts.slug, SUBSTRING(BlogPosts.content, 1, 150) AS content, BlogPosts.author_id, BlogPosts.category_ids, BlogPosts.created_at, BlogPosts.updated_at, BlogPosts.status, Users.name as author_name ` +
        `FROM BlogPosts ` +
